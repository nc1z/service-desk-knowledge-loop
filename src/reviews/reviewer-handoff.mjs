export const reviewerHandoffPath =
  "reviews/reviewer-handoff-vpn-login-failures.md";

export const reviewerHandoffFormat = {
  schemaVersion: 1,
  requiredSections: [
    "Review Scope",
    "Facts",
...
export const reviewerHandoffSections = reviewerHandoffFormat.requiredSections;

const evidence = [
  {
    ticket: "INC-2026-0001",
    source: "tickets/INC-2026-0001-vpn-token-refresh.md",
    summary:
      "Browser SSO succeeded, VPN failed after MFA approval, and the resolver cleared stale VPN authentication token state.",
  },
  {
    ticket: "INC-2026-0002",
    source: "tickets/INC-2026-0002-vpn-token-refresh.md",
    summary:
      "Device compliance and browser SSO were confirmed before auth-token mismatch was resolved by expiring cached IdP state.",
  },
  {
    ticket: "INC-2026-0003",
    source: "tickets/INC-2026-0003-vpn-token-refresh.md",
    summary:
      "Password reset was avoided because browser SSO worked; stale token state tied to a previous MFA device was removed.",
  },
  {
    ticket: "INC-2026-0004",
    source: "tickets/INC-2026-0004-vpn-token-refresh.md",
    summary:
      "Accepted MFA was followed by cached-token rejection; resolver notes mention device claim refresh and stale token cleanup.",
  },
  {
    ticket: "INC-2026-0013",
    source: "tickets/INC-2026-0013-vpn-false-positive-network.md",
    summary:
      "False-positive guardrail: VPN failed before MFA and was resolved through SSL fallback for a blocked UDP tunnel.",
  },
];

const changedFiles = [
  {
    path: "reviews/incident-pattern-brief-vpn-login-failures.md",
    summary:
      "Codex App-style incident pattern brief selecting the VPN login failures cluster and citing ticket evidence.",
  },
  {
    path: "runbooks/vpn-login-failures.md",
    summary:
      "Runbook update for post-MFA VPN authentication failures, stale token triage, evidence capture, and false-positive separation.",
  },
  {
    path: "scripts/check-vpn-session-demo.mjs",
    summary:
      "Safe local demo helper with dry-run behavior and non-dry-run blocking for VPN session evidence prompts.",
  },
  {
    path: "kb/vpn-login-failures.md",
    summary:
      "Draft-only service desk KB article covering symptoms, agent steps, user-facing wording, escalation triggers, and evidence references.",
  },
  {
    path: "reviews/runbook-update-record.md",
    summary:
      "Review record for the runbook path, rationale, ticket evidence, and no-production-access safety boundary.",
  },
  {
    path: "reviews/support-script-update-record.md",
    summary:
      "Review record for the support helper path, rationale, ticket evidence, and blocked production execution boundary.",
  },
  {
    path: reviewerHandoffPath,
    summary:
      "Phase 8 reviewer handoff packet for service desk lead and operations owner review.",
  },
];

export function buildReviewerHandoff() {
  const evidenceList = evidence
    .map((item) => `- ${item.ticket} (\`${item.source}\`): ${item.summary}`)
    .join("\n");
  const changedFileList = changedFiles
    .map((file) => `- \`${file.path}\`: ${file.summary}`)
    .join("\n");

  return `# Reviewer Handoff: VPN Login Failures Knowledge Loop
